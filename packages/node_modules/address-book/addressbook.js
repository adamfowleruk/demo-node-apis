...
if (parsedArgs.help) {
  ...
  console.log("Usage: address-book [-v] [-?] [--settings settings.js] [--userDir DIR]");
...
//     settingsFile = path.join(process.env.HOMEPATH, ".addressbook", "settings.js");
    ...
    var userDir = parsedArgs.userDir || path.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH, ".addressbook");
...
try {
  ADDRESSBOOK.start().then(function() {
    console.log("Address book start returned")
  });
} catch (err) {
